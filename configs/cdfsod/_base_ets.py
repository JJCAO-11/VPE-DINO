# =====================================================================
# CD-FSOD Benchmark - ETS Full Method Base Config
# ---------------------------------------------------------------------
# Inherits the GroundingDINO Swin-B model from mmdetection and applies
# the full ETS (Enhance-Then-Search) augmentation pipeline:
#     CachedMosaic + CachedMixUp + YOLOXHSVRandomAug + multi-scale
# This base reproduces the ETS paper recipe but with a single training
# run (no 50-group grid search). It is the version used to report the
# best ETS numbers on the CD-FSOD benchmark.
# =====================================================================
_base_ = [
...
# ---------------------------------------------------------------------
# Pipelines (with full ETS mix-augmentation)
# ---------------------------------------------------------------------
train_pipeline = [
    ...
    dict(type='CachedMixUp', img_scale=(640, 640),
         ratio_range=(1.0, 1.0), max_cached_images=10,
         pad_val=(114, 114, 114), prob=0.3),
    dict(
        type='RandomChoice',
        transforms=[
            [
...
#
# The previous warmup+cosine form still behaved badly on 1-shot runs with
# only 1 iter/epoch: after conversion, cosine captured the warmup-reduced
# LR as its base, so artaxor-1shot logged LR climbed toward 1e-5 instead
# of starting near the optimizer base LR and decaying.  A single cosine
# scheduler avoids that boundary interaction entirely.
#
# `eta_min_ratio=0.1` decays every parameter group to 10% of its own base
# LR, preserving the intended backbone lr_mult=0.1 and main/vpe_proj LR
