...
_C.DE.BG_PROTOTYPES = ""
_C.DE.TOPK = -1
_C.DE.SET_CD_VITO = False
_C.DE.ATTN_FUSE_RATIO = 0.7
_C.DE.TRAIN_TEST_OVERLAP = True
...
# ---------------------------------------------------------------------------- #
# LoRA (Low-Rank Adaptation) for DINOv2 backbone
# ---------------------------------------------------------------------------- #
_C.DE.USE_LORA = False           # Enable LoRA on DINOv2 backbone
_C.DE.LORA_RANK = 4             # LoRA rank (2 for 1-shot, 4 for 5-shot, 8 for 10-shot)
_C.DE.LORA_ALPHA = 1.0          # LoRA scaling factor
_C.DE.LORA_DROPOUT = 0.0        # LoRA dropout rate
_C.DE.LORA_TARGET_BLOCKS = None # List of block indices; None = last 1/3

# ---------------------------------------------------------------------------- #
# Multi-Head Cross-Attention Prototype Aggregator (方案B)
# ---------------------------------------------------------------------------- #
_C.DE.USE_MHCA_AGGREGATOR = False  # Replace InstReweight with MHCA Aggregator
_C.DE.MHCA_NUM_HEADS = 4          # Number of attention heads (must divide embed_dim)

# ---------------------------------------------------------------------------- #
# DGFM (Domain-Guided Feature Modulation) for post-backbone feature adaptation
# ---------------------------------------------------------------------------- #
_C.DE.USE_DGFM = False             # Enable DGFM after backbone output
_C.DE.DGFM_REP_SCALE = 4          # Reparameterization scale factor
_C.DE.DGFM_RESIDUAL_WEIGHT = 0.1  # Initial residual weight (learnable)