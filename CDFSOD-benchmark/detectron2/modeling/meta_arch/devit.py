...
from lib.lora import inject_lora_to_vit, count_lora_params
from lib.dgfm import DGFMWithResidual, count_dgfm_params
...
class MultiHeadCrossAttentionAggregator(nn.Module):
    """
    Multi-Head Cross-Attention Prototype Aggregator (方案B).

    替代 InstReweight，使用多头注意力聚合 K-shot 实例原型。

    原始 InstReweight 的问题：
    - ProtoCenterAttention 仅用 Linear(D,1) 计算注意力分数 → 表达能力弱
    - 不考虑实例间的关系（independent scoring）
    - 单维度 softmax 不能建模多方面的重要性

    本模块的改进：
    - 用类中心作为 Query，K 个实例作为 Key/Value
    - 多头注意力捕获多方面的原型质量信号
    - FFN 层增强非线性变换能力
    - 保留与原始 InstReweight 相同的 ratio 融合机制

    接口完全兼容 InstReweight:
        输入: x [N_cls, K_shot, D]
        输出: (aggregated [N_cls, D], attn_scores [N_cls, K_shot])
    """

    def __init__(self, c_in, ratio=0.5, num_heads=4, dropout=0.1, ffn_ratio=2.0):
        super(MultiHeadCrossAttentionAggregator, self).__init__()
        self.ratio = ratio
        self.c_in = c_in
        self.num_heads = num_heads
        assert c_in % num_heads == 0, f"c_in ({c_in}) must be divisible by num_heads ({num_heads})"
        self.head_dim = c_in // num_heads
        self.scale = self.head_dim ** -0.5

        # Layer norms
        self.norm_q = nn.LayerNorm(c_in)
        self.norm_kv = nn.LayerNorm(c_in)

        # Cross-attention projections
        self.q_proj = nn.Linear(c_in, c_in)
        self.k_proj = nn.Linear(c_in, c_in)
        self.v_proj = nn.Linear(c_in, c_in)
        self.out_proj = nn.Linear(c_in, c_in)
        self.attn_dropout = nn.Dropout(dropout)

        # FFN after attention (增强非线性变换)
        ffn_hidden = int(c_in * ffn_ratio)
        ...
        # 初始化：让初始输出接近零，保证初始行为近似原始 mean pooling
    ...
    def forward(self, x):
        """
        Args:
            x: [N_cls, K_shot, D] - K-shot instance prototypes per class

        Returns:
            out: [N_cls, D] - aggregated class prototypes
            attn_scores: [N_cls, K_shot] - attention weights per instance
        """
        N_cls, K, D = x.shape

        # 构造 Query: 类中心（K 个实例的均值）
        proto_mean = x.mean(dim=1)  # [N_cls, D]

        # Pre-norm
        q = self.norm_q(proto_mean)  # [N_cls, D]
        kv = self.norm_kv(x)         # [N_cls, K, D]

        # Multi-head cross-attention
        # Q: [N_cls, 1, num_heads, head_dim] -> [N_cls, num_heads, 1, head_dim]
        q = self.q_proj(q).view(N_cls, 1, self.num_heads, self.head_dim).transpose(1, 2)
        k = self.k_proj(kv).view(N_cls, K, self.num_heads, self.head_dim).transpose(1, 2)
        v = self.v_proj(kv).view(N_cls, K, self.num_heads, self.head_dim).transpose(1, 2)

        # Attention: [N_cls, num_heads, 1, K]
        attn_weights = (q @ k.transpose(-2, -1)) * self.scale
        attn_weights = F.softmax(attn_weights, dim=-1)
        attn_weights = self.attn_dropout(attn_weights)

        # 聚合: [N_cls, num_heads, 1, head_dim] -> [N_cls, D]
        x_attn = (attn_weights @ v).transpose(1, 2).reshape(N_cls, D)
        x_attn = self.out_proj(x_attn)

        # 残差连接: 注意力输出 + 原始均值
        x_attn = x_attn + proto_mean

        # FFN with residual
        x_attn = x_attn + self.ffn(x_attn)

        # 提取注意力分数用于可视化/分析: [N_cls, num_heads, 1, K] -> [N_cls, K]
        attn_scores = attn_weights.squeeze(2).mean(dim=1)  # 多头平均

        # 融合: ratio * 注意力聚合 + (1-ratio) * 简单平均 (与原始 InstReweight 保持一致)
        out = self.ratio * F.normalize(x_attn, dim=-1) + \
              (1 - self.ratio) * F.normalize(proto_mean, dim=-1)
        out = F.normalize(out, dim=-1)
...
class OpenSetDetectorWithExamples(nn.Module):
    ...
    def __init__(self,
                 ...
                 use_mhca_aggregator=False,
                 mhca_num_heads=4,
                 use_dgfm=False,
                 dgfm_rep_scale=4,
                 dgfm_residual_weight=0.1
                 ):
        super().__init__()
        ...
        # LoRA: defer injection until AFTER checkpoint loading
        # so that pretrained backbone weights can be loaded correctly.
        # Call apply_lora_injection() after resume_or_load().
        ...
        # DGFM: Domain-Guided Feature Modulation
        self.use_dgfm = use_dgfm
        self._dgfm_config = {
            'rep_scale': dgfm_rep_scale,
            'residual_weight': dgfm_residual_weight,
        }
        self.dgfm_module = None  # Will be initialized in apply_dgfm_init()
        ...
        if len(prototypes.shape) == 3:
            if self.set_cd_vito:
                ...
                if use_mhca_aggregator:
                    self.adapter = MultiHeadCrossAttentionAggregator(
                        c_in=prototypes.shape[-1],
                        ratio=attn_fuse_ratio,
                        num_heads=mhca_num_heads,
                        dropout=0.1,
                        ffn_ratio=2.0
                    ).cuda()
                    logging.getLogger(__name__).info(
                        f"[MHCA] Using MultiHeadCrossAttentionAggregator "
                        f"(heads={mhca_num_heads}, dim={prototypes.shape[-1]})"
                    )
                else:
    ...
    def apply_lora_injection(self):
        """Inject LoRA AFTER checkpoint loading so pretrained weights are preserved.
        
        Must be called after resume_or_load() to ensure backbone weights
        are properly loaded before being wrapped by LoRA layers.
        """
        if not self.use_lora:
            return
        cfg = self._lora_config
        self.lora_params = inject_lora_to_vit(
            self.backbone,
            rank=cfg['rank'],
            alpha=cfg['alpha'],
            dropout=cfg['dropout'],
        ...
        total_lora, trainable_lora = count_lora_params(self.backbone)
        logging.getLogger(__name__).info(
            f"[LoRA] Injected into DINOv2 backbone (post-load): "
            f"{total_lora:,} total LoRA params, {trainable_lora:,} trainable"
        )

    def apply_dgfm_init(self):
        """Initialize DGFM module after model construction.
        
        Called after resume_or_load() so it doesn't interfere with
        checkpoint loading. DGFM is a new module that won't have
        saved weights in checkpoints.
        """
        if not self.use_dgfm:
            return
        dgfm_cfg = self._dgfm_config
        self.dgfm_module = DGFMWithResidual(
            channels=self.ndim,
            rep_scale=dgfm_cfg['rep_scale'],
            residual_weight=dgfm_cfg['residual_weight']
        ).to(self.device)
        total_dgfm, trainable_dgfm = count_dgfm_params(self)
        logging.getLogger(__name__).info(
            f"[DGFM] Domain-Guided Feature Modulation initialized: "
            f"{total_dgfm:,} total params, {trainable_dgfm:,} trainable"
        )
    ...
    def from_config(cls, cfg, use_bn=False):
        ...
        # Freeze all backbone params first; LoRA will selectively unfreeze later
        ...
        return {
            ...
            # LoRA config
            "use_lora": cfg.DE.USE_LORA,
            "lora_rank": cfg.DE.LORA_RANK,
            "lora_alpha": cfg.DE.LORA_ALPHA,
            "lora_dropout": cfg.DE.LORA_DROPOUT,
            "lora_target_blocks": cfg.DE.LORA_TARGET_BLOCKS,

            # MHCA Aggregator config
            "use_mhca_aggregator": cfg.DE.USE_MHCA_AGGREGATOR,
            "mhca_num_heads": cfg.DE.MHCA_NUM_HEADS,

            # DGFM config
            "use_dgfm": cfg.DE.USE_DGFM,
            "dgfm_rep_scale": cfg.DE.DGFM_REP_SCALE,
            "dgfm_residual_weight": cfg.DE.DGFM_RESIDUAL_WEIGHT,
    ...
    def forward(self, batched_inputs: List[Dict[str, torch.Tensor]]):
        ...
        if self.use_lora:
            # LoRA mode: allow gradients to flow through LoRA params
            # but keep backbone in eval mode (BN/Dropout behavior unchanged)
        ...
        elif self.use_dgfm and self.dgfm_module is not None:
            # DGFM mode: backbone is frozen, but DGFM modulates features with gradients
            with torch.no_grad():
                if self.backbone.training: self.backbone.eval()
                with autocast(enabled=True):
                    all_patch_tokens = self.backbone(images.tensor)
                    patch_tokens = all_patch_tokens[self.vit_feat_name]
                    all_patch_tokens.pop(self.vit_feat_name)
            # Apply DGFM outside no_grad so its parameters receive gradients
            patch_tokens = self.dgfm_module(patch_tokens)
        else:
            # Original mode: fully frozen backbone, no gradients
            with torch.no_grad():
                if self.backbone.training: self.backbone.eval()
                with autocast(enabled=True):
                    all_patch_tokens = self.backbone(images.tensor)
