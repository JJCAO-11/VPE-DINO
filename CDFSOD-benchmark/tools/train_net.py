...
def main(args):
    ...
    # ===== CRITICAL: Inject LoRA AFTER checkpoint loading =====
    # This ensures pretrained backbone weights are loaded correctly
    # before being wrapped by LoRA layers.
    if cfg.DE.USE_LORA:
        model = trainer.model
        if hasattr(model, 'module'):  # Handle DistributedDataParallel
            model = model.module
        model.apply_lora_injection()

        # Rebuild optimizer to include the new LoRA parameters
        ...
        logging.getLogger("detectron2.trainer").info(
            "[LoRA] Optimizer rebuilt with LoRA parameters included."
        )

    # ===== DGFM: Initialize after checkpoint loading =====
    if cfg.DE.USE_DGFM:
        model = trainer.model
        if hasattr(model, 'module'):  # Handle DistributedDataParallel
            model = model.module
        model.apply_dgfm_init()

        # Rebuild optimizer to include the new DGFM parameters
        optimizer = torch.optim.SGD(
            [p for p in trainer.model.parameters() if p.requires_grad],
            lr=cfg.SOLVER.BASE_LR,
            momentum=cfg.SOLVER.MOMENTUM,
        ...
        trainer.scheduler = trainer.build_lr_scheduler(cfg, optimizer)
        logging.getLogger("detectron2.trainer").info(
            "[DGFM] Optimizer rebuilt with DGFM parameters included."
        )
